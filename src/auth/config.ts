// src/auth/config.ts
// Configuration loader for Google Calendar OAuth 2.0 credentials
// Follows mcp-gmail pattern for consistency
 ...
 * Priority: Environment variables > ~/.config/mcp-gcal/credentials.json > ./credentials.json
 */
export async function loadOAuthCredentials(): Promise<OAuthCredentials> {
  ...
  const envRedirectUri = process.env.GOOGLE_REDIRECT_URI || process.env.GCAL_REDIRECT_URI;
  ...
  // Priority: ~/.config/mcp-gcal/credentials.json > ./credentials.json (for backwards compatibility)
  try {
    ...
    try {
      ...
      credentialsPath = homeConfigPath;
    } catch {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new Error(
        ...
        '   (or GCAL_CLIENT_ID, GCAL_CLIENT_SECRET, GCAL_REDIRECT_URI)\n\n' +
        '2. Credentials File (Recommended):\n' +
        '   Place credentials.json in ~/.config/mcp-gcal/credentials.json\n' +
        '   (Or in the project root for backwards compatibility)\n' +
 ...
 * Uses ~/.config/mcp-gcal/ for system-wide tokens
 */
export function getTokenPath(): string {
  return join(homedir(), '.config', 'mcp-gcal', 'token.json');
}

/**
 * Get the path to the credentials directory
 * Uses ~/.config/mcp-gcal/ for system-wide storage
 */
export function getCredentialsDir(): string {
  return join(homedir(), '.config', 'mcp-gcal');
}
